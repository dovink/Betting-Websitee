...
const GamesList = ({ season, userRole, games, setGames }) => {
    ...
    const fetchGames = async () => {
      try {
        const response = await fetch(
          `http://localhost:5050/season/${season._id}/games`,
          {
            method: "GET",
            headers: {
            ...
            >
              <div className="flex items-center justify-between">
                {gameHasEnded ? (
                  <div className="flex items-center">
                    <span className="text-green-900 font-bold py-2">
                      {game.winner}
                    </span>
                    <span className="font-bold px-2 text-red-900">vs</span>
                    <span className="text-red-700 font-bold py-2">
                      {game.awayTeam}
                    </span>
                    <span className="ml-4">{game.winningMargin}</span>
                  </div>
                ) : (
                  <div className="flex items-center">
                    <span className="text-gray-500 font-bold py-2">
                      {game.homeTeam}
                    </span>
                    <span className="font-bold px-2 text-red-900">vs</span>
                    <span className="text-gray-500 font-bold py-2">
                ...
                <div className="absolute ml-96">
                  {userRole === "user" && !gameHasEnded && !season.Top4Updated &&(
                    <button
                      ...
                      onClick={() => handleOnWinnerClick(game._id)}
                    >
                      Įrašyti laimėtoją
                    </button>
                  )}
                ...
                <div className="absolute ml-64">
                  {userRole === "user" && !gameHasEnded && !season.Top4Updated &&(
                    <button
                      className="px-3 py-2 text-xs font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 "
                      onClick={() => handleOnGuessWinnerClick(game._id)}
                    >
                      Spėti laimėtoja
                    </button>
                  )}
                ...
                <div className="flex items-center">
                  <span className="font-medium pr-1 ">
                    {format(gameStartTime, "yyyy-MM-dd HH:mm", { locale: lt })}
                  </span>
                  <svg
                  ...
                  <span
                    className={`ml-2 py-1 rounded text-xs font-bold ${
                      gameHasEnded
                        ? "bg-red-700 text-black px-6"
                        : gameHasStarted
                        ? "bg-yellow-400 text-black px-10"
                        : "bg-green-500 text-black px-8"
                    }`}
                  >
                    {gameHasEnded
                      ? "Žaidimas pasibaige"
              ...
              {showUpdateWinnerForm === game._id && (
                <div className="mt-4">
                  ...
                  <UpdateWinnerForm
                    gameId={game._id}
                    teams={[game.homeTeam, game.awayTeam].map((team) => ({
                      value: team,
                      label: team,
                    }))}
