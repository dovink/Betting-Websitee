...
const TopScoreBoard = ({ seasonId }) => {
    ...
    useEffect(() => {
        const fetchTopUsers = async () => {
            try {
                const response = await fetch(`http://localhost:5050/footballSeason/${seasonId}/leaders`, {
                    method: 'GET',
                    headers: {
    ...
    return (
        <div className="w-3/12 mx-auto mt-10">
            {topUsers.map((item, index) => (
                <div key={index} className="flex items-center justify-between p-2 border border-gray-300 rounded-md mb-2 shadow-md">
                    ...
                    <div className="w-12 text-right font-bold text-2xl">{item.points}</div>
                </div>
            ))}
        </div>
      );
